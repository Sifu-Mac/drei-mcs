...
- Standardspalten-Umbenennung-Kandidat: `Eingang` wird als Backlog-Spalte zu `Produktionsplan`, `Kunden-Review` als Review-Spalte zu `DREI-Review` und `Änderungen angefordert` als Blocked-Spalte zu `Korrekturen`. Neue Standard-Review-Boards erhalten diese Namen; eine reversible Migration ändert nur exakt passende bisherige Standardspalten. Echter isolierter Reset sowie Up/Down-Nachweis erfolgreich; volle Suite `180 runs, 964 assertions, 0 failures, 0 errors`; unabhängiges `QA & Review`-Go. Deployment folgt dem Merge.
