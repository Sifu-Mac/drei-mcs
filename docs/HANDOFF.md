...
- Mail-Text-Deployment vorbereitet: Absendername, Betreffe und Inhalte der Einladungs- und Passwort-Reset-E-Mails lauten nun `DB × DREI` und sind auf Deutsch. Die gemeinsame Fußzeile beschreibt DB × DREI als digitales Kollaborations- und Freigabetool der Digitalbackup MW GmbH. Mailer-Tests `2 runs, 20 assertions` und volle isolierte Suite `180 runs, 974 assertions, 0 failures, 0 errors` grün; unabhängiges `QA & Review`-Go ohne Befunde. Das Runtime-Deployment steht noch aus.
