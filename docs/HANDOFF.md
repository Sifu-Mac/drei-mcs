...
- Aktueller, noch nicht gemergter GoLive-Kandidat: `codex/golive-version-label` auf Basis der getrennten Commits `d332bff` (Workflow), `09b4f3f` (CSV-Export) und `1b9e8dc` (Versionshinweis). Er macht `Freigegeben` zur Review-Stufe, ergänzt `Fertig` als finale Spalte, entfernt den ausgegrauten Abschlussstil, fügt einen sicheren CSV-Boardexport hinzu und zeigt im Profilmenü `DREI Asset Review · GoLive v1.0.0`.
- Das Zugriffsmodell für GoLive ist explizit mit Tests abgesichert: Owner, Member und Client sehen alle aktiven Kampagnen und Boards ihres gemeinsamen Workspaces; fremde Workspaces bleiben getrennt.
- Verifiziert im isolierten GoLive-Teststack: vollständige Rails-Suite `162 runs, 836 assertions, 0 failures, 0 errors`; RuboCop `175 files inspected, no offenses`; Brakeman `0 security warnings`; Dependency-Audit ohne bekannte Schwachstellen. Vor Merge/Deployment steht das unabhängige `QA & Review`-Go aus.
- Der erste GoLive-QA-Lauf fand einen Migrations-Kollisionsfall: Eine bereits frei benannte Spalte `Fertig` ohne `done`-Art verhinderte die garantierte finale Spalte. Der Kandidat `0f5edaf` konvertiert diese Spalte jetzt explizit zu `done`, synchronisiert ihre Karten und deckt den Fall mit einem Regressionstest ab. Zusätzlich testen Exportrechte nun Client, Owner und Member sowie die Boardansicht auf das vollständige Fehlen des alten Done-Graustils. Fokussierte Nachtests: `21 runs, 134 assertions, 0 failures, 0 errors`; erneutes unabhängiges QA-Review steht aus.
- Vollständige Wiederholungsprüfung auf dem korrigierten Kandidaten: Rails `164 runs, 847 assertions, 0 failures, 0 errors`; RuboCop `175 files inspected, no offenses`; Brakeman `0 security warnings`; Dependency-Audit ohne bekannte Schwachstellen.
- Der unabhängige QA-Thread hat am 2026-07-31 für `fafe786` ausdrücklich Go erteilt: From-scratch-Migration, Kollisions-Regression, gezielte Migration-/Export-/Viewtests `21 runs, 134 assertions`, vollständige Suite `164 runs, 847 assertions`, Selenium `6 runs, 37 assertions`, RuboCop, Brakeman sowie Dependency-/Importmap-Audits sind grün.
- Der Stand wurde danach als Merge-Commit `df5e1e3` nach `main` gemergt, zu GitHub gepusht und in Production ausgerollt. `db` ist healthy, `web` aktiv, `20260731110000 Make finished the final board column` ist `up` und `https://drei.digitalbackup.cloud/up` liefert `200`.
...
- Am 2026-07-31 wurde nach ausdruecklicher Freigabe genau ein temporaerer Client-Invite an die eigene Adresse `hello@digitalbackup.at` angelegt und ueber den regulaeren `InviteMailer` sowie `MailDeliveryJob` versendet. Der Queue-Job wurde ohne Fehler abgeschlossen und `/up` blieb `200`. Der Benutzer hat den Invite angenommen und ein Passwort gesetzt. Serverseitig verifiziert: Konto vorhanden, kein Admin, genau eine Workspace-Mitgliedschaft mit Rolle `client`. Client-Login, Browser-Rechtepruefung und anschliessende gezielte Bereinigung stehen noch aus; der Invite-Token wurde weder ausgegeben noch dokumentiert.
