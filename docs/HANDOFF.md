...
- Auth-Formular-Fokuskorrektur bereit für Deployment: Login, Passwort vergessen, Passwort-Reset und Invite-Aktivierung verwenden nun einen einzelnen gedämpften Fokusindikator am Feldcontainer. Der globale harte Innenrahmen wird nur für diese Eingaben entfernt; Tastaturfokus und native Pflichtfeldvalidierung bleiben erhalten. Fokussierte Browser-Tests `2 runs, 5 assertions`, betroffene Controller-Tests `17 runs, 68 assertions`, vollständige isolierte Suite `194 runs, 1044 assertions` und RuboCop für die geänderten Tests sind grün; unabhängiges `QA & Review`-Go ohne Befunde. Das Runtime-Deployment steht noch aus.
